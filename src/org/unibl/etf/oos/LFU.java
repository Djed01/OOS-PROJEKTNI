            ...
            if(i==0){
                ...
                for(int j=3;j<brojOkvira+2;j++)
                    ...
                    for(int j=2;j<brojOkvira+2;j++) {
                        if ( (matrica[j][i-1]!="  ") && (matrica[j][i-1]!="0") &&  (Integer.parseInt(matrica[j][i - 1]) != nizReferenci[i]) ) {
                            mapa.put(Integer.parseInt(matrica[j][i - 1]), mapa.get(Integer.parseInt(matrica[j][i - 1])) - 1);
                        } else if((matrica[j][i-1]!="  ") && (matrica[j][i-1]!="0") && (Integer.parseInt(matrica[j][i - 1]) == nizReferenci[i])) {
